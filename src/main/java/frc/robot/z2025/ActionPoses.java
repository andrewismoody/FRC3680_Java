...
public class ActionPoses {
    public static void Initialize(SwerveDriveModule swerveDriveModule, SingleMotorModule elevator, SingleActuatorModule slide) {
        ...
        swerveDriveModule.AddActionPose(new ActionPose(Group.Start, Location.Barge.getValue(), 1, Position.Any.getValue(), Action.Any,
            new AutoTarget(Constants.getKnownRotation(Location.Tag, 20).plus(Rotation2d.fromDegrees(180)))));
        // waypoint 11, Lookat reef
        swerveDriveModule.AddActionPose(new ActionPose(Group.Travel, Location.Waypoint.getValue(), 11, Position.Any.getValue(), Action.Any,
            new AutoTarget(Constants.getFieldPosition(Location.Waypoint, 11), Constants.getFieldPosition(Location.Interest, 1))));
        ...
        swerveDriveModule.AddActionPose(new ActionPose(Group.Travel, Location.Waypoint.getValue(), 12, Position.Any.getValue(), Action.Any,
            new AutoTarget(Constants.getFieldPosition(Location.Waypoint, 12), Constants.getFieldPosition(Location.Interest, 1))));
