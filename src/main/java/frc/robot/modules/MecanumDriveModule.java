...
public class MecanumDriveModule implements DriveModule {
    String moduleID;
    MecanumMotorModule[] driveModules;
    double driveSpeed;
    double driveRatio;
...
//    StructArrayPublisher<SwerveModuleState> swerveModuleCommandedPublisher;
    ...
    public MecanumDriveModule(String ModuleID, Gyro Gyro, Positioner Positioner, double DriveSpeed, double DriveRatio, double RotationSpeed,
            double FloatTolerance, MecanumMotorModule ... modules) {
        moduleID = ModuleID;
        ...
        driveModules = new MecanumMotorModule[modules.length];
        Translation2d leftFrontTranslation = Translation2d.kZero;
        Translation2d rightFrontTranslation = Translation2d.kZero;
        Translation2d leftRearTranslation = Translation2d.kZero;
        Translation2d rightRearTranslation = Translation2d.kZero;
        for (MecanumMotorModule module : modules) {
            var i = module.GetSwervePosition().getValue();
            driveModules[i] = module;
            module.setDriveModule(this);

            var pos = module.GetSwervePosition();
            switch (pos) {
                case LeftFront:
                    leftFrontTranslation = module.modulePosition;
                    break;
                case RightFront:
                    rightFrontTranslation = module.modulePosition;
                    break;
                case LeftRear:
                    leftRearTranslation = module.modulePosition;
                    break;
                case RightRear:
                    rightRearTranslation = module.modulePosition;
                    break;
            }
        }
        ...
        kinematics = new MecanumDriveKinematics(leftFrontTranslation, rightFrontTranslation, leftRearTranslation, rightRearTranslation);
        odometry = new MecanumDriveOdometry(kinematics, new Rotation2d(getGyroRadians()), positions);
        fieldPosition = new Field2d();

        poseEstimator = new MecanumDrivePoseEstimator(kinematics, new Rotation2d(getGyroRadians()), positions, Pose2d.kZero);
    }
    ...
    public void ResetEncoders() {
        for (MecanumMotorModule module : driveModules) {
    ...
    public void SetCurrentPose(Pose3d newPose) {
        double[] positions = new double[driveModules.length];
    ...
    public void ProcessState(boolean isAuto) {
        ...
        for (MecanumMotorModule module : driveModules) {
            ...
            switch (pos) {
                case LeftFront:
                    module.updateModuleValues(moduleSpeeds.frontLeftMetersPerSecond);
                    positions.frontLeftMeters = module.getPosition();
                    break;
                case RightFront:
                    module.updateModuleValues(moduleSpeeds.frontRightMetersPerSecond);
                    positions.frontRightMeters = module.getPosition();
                    break;
                case LeftRear:
                    module.updateModuleValues(moduleSpeeds.rearLeftMetersPerSecond);
                    positions.rearLeftMeters = module.getPosition();
                    break;
                case RightRear:
                    module.updateModuleValues(moduleSpeeds.rearRightMetersPerSecond);
                    positions.rearRightMeters = module.getPosition();
                    break;
            }
            var i = pos.getValue();
